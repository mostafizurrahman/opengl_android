...
public final class GlUtil {
    ...
    public static int createProgram(Context context, String vertexAssetFile, String fragmentAssetFile) {
        ...
        Log.d("vertexShader log: %s", GLES20.glGetShaderInfoLog(vertexShader));

        int fragmentShader = compileShader(GLES20.GL_FRAGMENT_SHADER, fragmentSource);
        if (fragmentShader == 0) {
            return 0;
        }
        Log.d("fragmentShader log: %s", GLES20.glGetShaderInfoLog(fragmentShader));

        int program = GLES20.glCreateProgram();
        checkGLError("glCreateProgram");
        if (program == 0) {
            Log.e("","Could not create program from "+ fragmentAssetFile +", " + vertexAssetFile);
        } else {
    ...
    private static int compileShader(int shaderType, String source) {
        ...
        if (compiled[0] == 0) {
//            Log.e("Could not compile shader %d", shaderType);
